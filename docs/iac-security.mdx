...
description: Scan Terraform, Kubernetes manifests, and cloud configuration before deployment.
---
...
Scan Terraform, Kubernetes manifests, and cloud configuration before deployment.
  ...
  security-check:
    ...
    steps:
      ...
      - name: Run security control
        run: |
          echo "Run IaC Security checks here"
...
IaC Security is not only a tool topic. It is part of secure software delivery. Start with the risk, place the control in the right CI/CD stage, and make feedback easy for developers to act on.
