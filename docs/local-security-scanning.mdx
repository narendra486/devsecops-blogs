...
description: Fast developer feedback using local scanning before code reaches GitHub.
---
...
Fast developer feedback using local scanning before code reaches GitHub.
  ...
  security-check:
    ...
    steps:
      ...
      - name: Run security control
        run: |
          echo "Run Local Security Scanning checks here"
...
Local Security Scanning is not only a tool topic. It is part of secure software delivery. Start with the risk, place the control in the right CI/CD stage, and make feedback easy for developers to act on.
