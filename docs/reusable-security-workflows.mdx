...
description: Centralize security checks so AppSec and platform teams can scale across many repositories.
---
...
Centralize security checks so AppSec and platform teams can scale across many repositories.
...
```yaml
name: reusable-security-workflows
  ...
  security-check:
    ...
    steps:
      ...
      - name: Run security control
        run: |
          echo "Run Reusable Security Workflows checks here"
...
Reusable Security Workflows is not only a tool topic. It is part of secure software delivery. Start with the risk, place the control in the right CI/CD stage, and make feedback easy for developers to act on.
