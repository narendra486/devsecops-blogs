...
description: Use short-lived cloud credentials instead of long-lived secrets in CI/CD.
---
...
Use short-lived cloud credentials instead of long-lived secrets in CI/CD.
  ...
  security-check:
    ...
    steps:
      ...
      - name: Run security control
        run: |
          echo "Run OIDC and Cloud Security checks here"
...
OIDC and Cloud Security is not only a tool topic. It is part of secure software delivery. Start with the risk, place the control in the right CI/CD stage, and make feedback easy for developers to act on.
