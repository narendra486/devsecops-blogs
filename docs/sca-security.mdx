...
description: Dependency security for vulnerable packages, transitive dependencies, and malicious package risks.
---
...
Dependency security for vulnerable packages, transitive dependencies, and malicious package risks.
  ...
  security-check:
    ...
    steps:
      ...
      - name: Run security control
        run: |
          echo "Run SCA Security checks here"
...
SCA Security is not only a tool topic. It is part of secure software delivery. Start with the risk, place the control in the right CI/CD stage, and make feedback easy for developers to act on.
