...
description: Use pre-commit and Git hooks to stop obvious mistakes before Git history is created.
---
...
Use pre-commit and Git hooks to stop obvious mistakes before Git history is created.
  ...
  security-check:
    ...
    steps:
      ...
      - name: Run security control
        run: |
          echo "Run Pre-Commit Security checks here"
...
Pre-Commit Security is not only a tool topic. It is part of secure software delivery. Start with the risk, place the control in the right CI/CD stage, and make feedback easy for developers to act on.
