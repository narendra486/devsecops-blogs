...
description: PRs as the main policy enforcement point for SAST, SCA, secrets, IaC, and container checks.
---
...
PRs as the main policy enforcement point for SAST, SCA, secrets, IaC, and container checks.
  ...
  security-check:
    ...
    steps:
      ...
      - name: Run security control
        run: |
          echo "Run Pull Request Security checks here"
...
Pull Request Security is not only a tool topic. It is part of secure software delivery. Start with the risk, place the control in the right CI/CD stage, and make feedback easy for developers to act on.
